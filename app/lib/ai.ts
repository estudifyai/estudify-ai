...
export async function generateText({
  ...
  return generateGroq({ system, messages, maxTokens, json });
}

/* ─── Groq (API compatible con OpenAI) ─── */
async function generateGroq({ system, messages, maxTokens, json }: GenerateOpts) {
...
export function parseJsonArray(raw: string, key?: string): any[] {
  if (!raw) return [];
  const cleaned = raw.replace(/```json/gi, "").replace(/```/g, "").trim();

  const pick = (val: any): any[] => {
  ...
  try {
    return pick(JSON.parse(cleaned));
  } catch {
    for (const [open, close] of [["{", "}"], ["[", "]"]] as const) {
      ...
      if (start !== -1 && end > start) {
        try {
          return pick(JSON.parse(cleaned.slice(start, end + 1)));
        } catch {}
      }
    }
    return [];
